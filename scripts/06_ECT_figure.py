# scripts/04_ECT_fig.py (MODIFIED)
...
# --- MODIFIED CLASS LABELS (6 classes) ---
CLASS_LABELS = [
    "algeria",
    "rootstock",
    "vinifera",
...
# --- 3. DATA LOADING AND PREPARATION (MODIFIED CLASSIFICATION) ---
...
# Function to collapse Algeria varieties into one class and handle 'mision'
def simplify_class_label(label):
    algeria_vars = ["Ahmeur Bou Ahmeur", "Amer Bouamar", "Babari", "Bouabane des Aures", 
                    "Ichmoul", "Ichmoul Bacha", "Louali", "Tizi Ouinine"]
    
    if label in algeria_vars:
        return "algeria"
    # Assuming 'mision' is already present or correctly added to the metadata
    elif label == "mision":
        return "mision"
    # Keep the other classes as is
    return label

df_valid['simplified_class_label'] = df_valid['class_label'].apply(simplify_class_label)

# Filter the DataFrame to only include the new 6 classes
df_valid = df_valid[df_valid['simplified_class_label'].isin(CLASS_LABELS)].copy()
print(f"Filtered to {len(df_valid)} leaves for the 6 target classes.")

# Ensure output directory exists
if not os.path.exists(OUTPUT_DIR):
    os.makedirs(OUTPUT_DIR)

# --- 4. CREATE FIGURE AND PLOT PANELS (MODIFIED LAYOUT) ---

# Set up the figure and a GridSpec with an extra column for the row labels
# Total columns remain 13 (1 for label + 6 for real + 6 for synthetic)
# Total rows are now NUM_CLASSES (6)
fig = plt.figure(figsize=(8.5, 4.5)) # Adjusted height for 6 rows
gs = gridspec.GridSpec(NUM_CLASSES, 13, figure=fig, wspace=0.01, hspace=0.01)
...
# Loop through each new class label to fill the rows
for row_idx, class_label in enumerate(CLASS_LABELS):
    ...
    # Filter data for the current simplified class
    class_df = df_valid[df_valid['simplified_class_label'] == class_label]
    ...
    for i in range(3):
        if i < len(real_leaves):
            ...
            # Blade representation (Cols 1, 3, 5)
            ...
            # Vein representation (Cols 2, 4, 6)
            ...
            # Placeholder for missing real data
    ...
    for i in range(3):
        if i < len(synthetic_leaves):
            ...
            # Blade representation (Cols 7, 9, 11)
            blade_ax = fig.add_subplot(gs[row_idx, 7 + 2*i])
            plot_leaf_panel(blade_ax, leaf['file_blade_ect'].split('/')[-2], leaf['file_blade_ect'].split('/')[-1], leaf['file_blade_mask'].split('/')[-2], leaf['file_blade_mask'].split('/')[-1])
            # Vein representation (Cols 8, 10, 12)
            vein_ax = fig.add_subplot(gs[row_idx, 7 + 2*i + 1])
            plot_leaf_panel(vein_ax, leaf['file_vein_ect'].split('/')[-2], leaf['file_vein_ect'].split('/')[-1], leaf['file_vein_mask'].split('/')[-2], leaf['file_vein_mask'].split('/')[-1])
        else:
            # Placeholder for missing synthetic data
...
# This must span the new number of rows (6)
